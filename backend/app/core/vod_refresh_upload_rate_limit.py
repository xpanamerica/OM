"""VOD RefreshUploadVideo：按用户滑动 60s 窗口限流。

优先 **Redis ZSET**（与评论发帖限流同 Lua 语义，多实例 / 多 worker 一致），不可用时按配置回退进程内 deque。
"""
...
RL_KEY_PREFIX = "rl:vod:refresh:v1"
...
def reset_vod_refresh_upload_rate_limit_for_tests() -> None:
    with _lock:
        _timestamps.clear()


def purge_vod_refresh_upload_rate_limit_redis_keys_for_tests() -> None:
...
def _try_consume(bucket_key: str, *, max_per_minute: int) -> tuple[bool, int | None]:
    if max_per_minute <= 0:
        return True, None
    window = 60.0
    use_redis = settings.VOD_REFRESH_UPLOAD_RATE_LIMIT_USE_REDIS and redis_available_cached()
    if use_redis:
        try:
            return _consume_redis(bucket_key, max_per_minute, window)
        except Exception as e:
            if settings.VOD_REFRESH_UPLOAD_RATE_LIMIT_REDIS_FALLBACK_MEMORY:
                logger.warning(
                    "vod_refresh_upload_rate_limit redis failed; memory fallback: %s",
                    e,
                    exc_info=False,
                )
...
def consume_vod_refresh_upload_slot_or_raise(*, user_id: uuid.UUID) -> None:
    limit = settings.VOD_REFRESH_UPLOAD_MAX_PER_USER_PER_MINUTE
    if limit <= 0:
        return
    ok, retry_after = _try_consume(str(user_id), max_per_minute=limit)
    if ok:
        return
    from app.infrastructure.observability.upload_flow_metrics import inc_vod_refresh_upload_rate_limited

    inc_vod_refresh_upload_rate_limited()
    ra = str(retry_after if retry_after is not None else 60)
    raise AppError(
        "上传凭证刷新过于频繁，请稍后再试",
        status_code=429,
        code=VOD_REFRESH_UPLOAD_RATE_LIMITED,
        headers={"Retry-After": ra},
    )


def vod_refresh_rate_limit_storage_label() -> str:
    if settings.VOD_REFRESH_UPLOAD_MAX_PER_USER_PER_MINUTE <= 0:
        return "disabled"
    if settings.VOD_REFRESH_UPLOAD_RATE_LIMIT_USE_REDIS and redis_available_cached():
        return "redis"
    return "memory"


def vod_refresh_rate_limit_operational(*, redis_connected: bool | None = None) -> bool:
    """严格 Redis 模式（禁止内存回退）下，当前是否具备可用的限流后端。"""
    if settings.VOD_REFRESH_UPLOAD_MAX_PER_USER_PER_MINUTE <= 0:
        return True
    if not settings.VOD_REFRESH_UPLOAD_RATE_LIMIT_USE_REDIS:
        return True
    if settings.VOD_REFRESH_UPLOAD_RATE_LIMIT_REDIS_FALLBACK_MEMORY:
        return True
