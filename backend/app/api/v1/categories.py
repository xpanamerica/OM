...
def list_categories(
    ...
    items, has_more = category_service.list_categories(
        db, limit=limit, offset=offset, cursor=cursor
    )
...
def create_category(db: DbSession, body: CategoryCreate) -> CategoryPublic:
    row = category_service.create_category(db, body)
...
@router.patch(
    ...
    response_model=CategoryPublic,
    summary="更新分类",
    description="需管理员 JWT。可部分更新；若修改 `name` 则不得与已有分类重名。",
    responses={**UNAUTH_OR_FORBIDDEN, **NOT_FOUND_CATEGORY, **NAME_CONFLICT},
    dependencies=_admin_write,
)
def update_category(
    db: DbSession,
    ...
    row = category_service.update_category(db, category_id, body)
    return CategoryPublic.model_validate(row)
...
@router.delete(
    ...
    responses={**UNAUTH_OR_FORBIDDEN, **NOT_FOUND_CATEGORY},
    dependencies=_admin_write,
)
def delete_category(
    response: Response,
