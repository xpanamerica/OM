...
export function FavoritesScreen(_props: Props) {
  ...
  const [items, setItems] = useState<Awaited<ReturnType<typeof usersApi.listMyFavoriteVideos>>["items"]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const load = useCallback(async () => {
    if (!token) return;
    const r = await usersApi.listMyFavoriteVideos({ offset: 0, limit: 50 }, token);
    setItems(r.items);
  }, [token]);
  ...
  return (
    <View style={styles.screen}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable style={styles.row} onPress={() => rootNav?.navigate("Detail", { id: item.id })}>
            <Text style={styles.rowTitle} numberOfLines={2}>
              {item.title}
            </Text>
            <Text style={styles.rowMeta}>
              {item.views_count} 播放 · {item.likes_count} 赞
            </Text>
          </Pressable>
        )}
