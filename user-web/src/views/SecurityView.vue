...
async function disable() {
  try {
    await authApi.disableMfa(disableCode.value.trim());
    disableCode.value = "";
...
<template>
  <div v-loading="loading" class="security">
    <h2>账号安全</h2>
    <el-card class="card">
      <template #header>MFA 二次验证</template>
      ...
      <div v-if="mfa?.enabled" class="disable-row">
        <el-input v-model="disableCode" class="code" placeholder="验证码或恢复码" />
        <el-button type="danger" plain @click="disable">关闭 MFA</el-button>
      </div>
    </el-card>
