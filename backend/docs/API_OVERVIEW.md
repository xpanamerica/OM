...
| POST | `P/auth/register` | JSON：`email`、`username`、`password`（≥8 字符）、`turnstile_token`；可选 `invite_code`。Turnstile 校验失败返回 400；当后台开启内测邀请码时，未填或无效码返回 400 及对应文案；201 返回用户公开信息 |
| POST | `P/auth/login` | `application/x-www-form-urlencoded`：`username`（可填用户名或邮箱）、`password`；同一登录标识失败达到 `TURNSTILE_LOGIN_FAILURE_THRESHOLD` 后还须提交 `turnstile_token`；200 返回 `access_token`、`token_type` |
...
**注册与邀请码（补充）**：`AUTH_REGISTER_MAX_ATTEMPTS_PER_MINUTE` 默认为 **0**（不启用），此时「每分钟」注册上限完全由 `AUTH_RATE_LIMIT_REGISTER_PER_IP_PER_MINUTE` 决定；设为 **20–60** 等正值时**仅覆盖该分钟维度**（小时仍用 `AUTH_RATE_LIMIT_REGISTER_PER_IP_PER_HOUR`），便于生产单独调参。邀请码消费使用数据库条件 `UPDATE`（`status='active'`、未过期、`used_count < max_uses` 才递增），因此即使两个请求同时通过前置读取，最终也只能有一个成功消费一次性邀请码；仓储层用例 `test_invite_atomic_redeem_allows_only_one_success` 覆盖该不变量。真实 HTTP 进程级并发用例为 `RUN_INVITE_HTTP_CONCURRENCY=1 pytest tests/integration/test_registration_invite_http_concurrency.py`；默认用临时 SQLite 文件库，本地可通过 `INVITE_HTTP_CONCURRENCY_DATABASE_URL` 指定外部库。CI job **invite-http-concurrency** 使用 PostgreSQL service、先跑 `alembic upgrade head`，再固定执行该 HTTP 并发用例；本地不应依赖 `TestClient` 多线程模拟该场景。

认证限流（`AUTH_RATE_LIMIT_ENABLED=true` 且 Redis 可用时多实例一致）：默认配额为注册每 IP 每分钟 3 / 每小时 10（若 `AUTH_REGISTER_MAX_ATTEMPTS_PER_MINUTE>0` 则「每分钟」以前者为准，见上节）；登录每 IP 每分钟 5；同一登录标识 15 分钟内失败 5 次后拒绝，失败达到 `TURNSTILE_LOGIN_FAILURE_THRESHOLD`（默认 3）后必须先通过 Turnstile；忘记密码每 IP 每小时 3、每邮箱每小时 2。各维度滑动窗口秒数（如 `AUTH_RATE_LIMIT_REGISTER_MINUTE_WINDOW_SECONDS`）与配额均可经环境变量覆盖（见 `app/core/config.py`）。Redis 路径使用 ``SCRIPT LOAD`` + ``EVALSHA``（键前缀 ``rl:auth:v2:``，含窗口的维度如 ``login:fail:w{秒}:``）；进程内曾判定 Redis 不可达时，可按 `AUTH_RATE_LIMIT_REDIS_STALE_REPROBE_SECONDS` 周期性重探测。Turnstile 服务端 secret 只从 `TURNSTILE_SECRET` 读取；注册/登录分别校验 Turnstile `action=register/login`，可用 `TURNSTILE_ALLOWED_HOSTNAMES` 限制 Siteverify 返回的 hostname；`TURNSTILE_BYPASS=true` 仅允许 `NODE_ENV=test`。`EXPOSE_PROMETHEUS_METRICS=true` 时 ``GET /metrics`` 含 ``app_auth_rate_limit_exceeded_total{kind=...}``、``app_auth_rate_limit_redis_errors_total``、``app_auth_rate_limit_memory_fallback_total``。可选真 Redis 集成测试：`RUN_AUTH_RATE_LIMIT_REDIS=1 pytest tests/integration/test_auth_rate_limit_redis.py`；CI 中由 workflow job **auth-rate-limit-redis** 固定执行。超限事件写入 `security_events` 表。
